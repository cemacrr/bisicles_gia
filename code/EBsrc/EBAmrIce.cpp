...
Real
EBAmrIce::computeTotalIce() const
{
...
void
EBAmrIce::getIceThickness(Real* a_data_ptr, int* a_dim_info, 
...
bool 
EBAmrIce::isDefined() const
{
  return m_is_defined;
}


EBAmrIce::EBAmrIce() : m_velSolver(NULL),
...
void
EBAmrIce::setThicknessBC( IceThicknessIBC* a_thicknessIBC)
{
...
void 
EBAmrIce::setTemperatureBC( IceTemperatureIBC* a_temperatureIBC)
{
  m_temperatureIBCPtr = a_temperatureIBC->new_temperatureIBC();
}

void 
EBAmrIce::defineSolver()
{
...
void
EBAmrIce::run(Real a_max_time, int a_max_step)
{

  if (s_verbosity > 3) 
    {
      pout() << "EBAmrIce::run -- max_time= " << a_max_time 
	      ...
	      && (dt > TIME_EPS))
	{
	  ...
	  if ((m_cur_step%m_check_interval == 0) && (m_check_interval > 0)
	      ...
	      if (m_cur_step > 0 && m_check_exit)
		{
		  if (s_verbosity > 2)
		    {
		      pout() << "EBAmrIce::exit on checkpoint" << endl;
		      return;
		    }
		}
  ...
  if (s_verbosity > 2)
    {
      pout() << "EBAmrIce::run finished" << endl;
    }
}


void
EBAmrIce::timeStep(Real a_dt)
{
  ...
  if (m_temporalAccuracy < 3)
      ...
      for (int lev=0; lev<= finestTimestepLevel();  lev++)
        {
          ...
          if (advectPhysPtr == NULL)
            {
              MayDay::Error("EBAmrIce::timestep -- unable to upcast GodunovPhysics to AdvectPhysics");
            }
    ...
    {
      MayDay::Error("EBAmrIce::timestep -- RK3 not implemented");
    }
    ...
    {
      MayDay::Error("EBAmrIce::timestep -- un-defined temporal accuracy");
    }
  ...
  if (s_verbosity > 0) 
    {
      pout () << "EBAmrIce::timestep " << m_cur_step
              << " --     end time = " 
...
EBAmrIce::regrid()
{
  ...
  CH_TIME("EBAmrIce::regrid");

  if (s_verbosity > 3) 
    { 
      pout() << "EBAmrIce::regrid" << endl;
    }
	...
	if (gridsSame)
	  {
	     if (s_verbosity > 3) 
	       { 
		 pout() << "EBAmrIce::regrid -- grids unchanged" << endl;
	       }
	     //return;
	  }
...
void 
EBAmrIce::tagCells(Vector<IntVectSet>& a_tags)
{
  
  if (s_verbosity > 3) 
    { 
      pout() << "EBAmrIce::tagCells" << endl;
    }
  ...
  // if (s_verbosity > 3) 
  //   { 
  //     pout() << "EBAmrIce::tagCells, subset II" << endl;
  //   }
...
void
EBAmrIce::tagCellsLevel(IntVectSet& a_tags, int a_level)
{

  if (s_verbosity > 4) 
    { 
      pout() << "EBAmrIce::tagCellsLevel " << a_level << endl;
    }
...
void
EBAmrIce::tagCellsInit(Vector<IntVectSet>& a_tags)
{

  if (s_verbosity > 3) 
    { 
      pout() << "EBAmrIce::tagCellsInit" << endl;
    }
...
void
EBAmrIce::initGrids(int a_finest_level)
{

  if (s_verbosity > 3) 
    { 
      pout() << "EBAmrIce::initGrids" << endl;
    }
...
void
EBAmrIce::setupFixedGrids(const std::string& a_gridFile)
{
...
void
EBAmrIce::levelSetup(int a_level, const DisjointBoxLayout& a_grids)
{
  IntVect ghostVect = IntVect::Unit;
...
void
EBAmrIce::initData(Vector<RefCountedPtr<LevelSigmaCS> >& a_vectCoordSys,
                 Vector<LevelData<FArrayBox>* >& a_velocity)
{

  if (s_verbosity > 3) 
    { 
      pout() << "EBAmrIce::initData" << endl;
    }
...
void EBAmrIce::solveVelocityField(Vector<LevelData<FArrayBox>* >& a_velocity,
                                  Real                            a_convergenceMetric)
{
  ...
  // recompute the viscous tensor field
  m_viscousTensor_valid = false;
  ...
  for (int lev=0; lev<=m_finest_level; lev++)
    {
	  ...
	  if(m_groundingLineCorrection && anyFloating == 1 && 0.0 * m_basalLengthScale < dx[0])
	    {
  ...
  if (m_basalLengthScale > TINY_NORM)
    {
      ...
      helmholtzSolve(m_velBasalC,
                     Real(1.0),
                     std::pow(m_basalLengthScale,2));
...
void
EBAmrIce::setMuCoefficient(Vector<LevelData<FArrayBox>* >& a_cellMuCoef, 
...
void
EBAmrIce::setBasalFriction(Vector<LevelData<FArrayBox>* >& a_vectBeta)
{
...
void 
EBAmrIce::computeFaceVelocity(Vector<LevelData<FluxBox>* >& a_faceVelAdvection, 
...
void
EBAmrIce::computeDivThicknessFlux(Vector<LevelData<FArrayBox>* >& a_divFlux,
...
void
EBAmrIce::incrementWithDivFlux(Vector<LevelData<FArrayBox>* >& a_phi,
...
void
EBAmrIce::updateCoordSysWithNewThickness(const Vector<LevelData<FArrayBox>* >& a_thickness)
{
...
Real 
EBAmrIce::computeInitialDt()
{

  if (s_verbosity > 3) 
    { 
      pout() << "EBAmrIce::computeInitialDt" << endl;
    }
 ...
 */
void EBAmrIce::updateGroundingLineProximity() const
{

  CH_TIME("EBAmrIce::updateGroundingLineProximity");
...
const LevelData<FArrayBox>* EBAmrIce::viscousTensor(int a_level) const
{
  updateViscousTensor();
  if (!(m_viscousTensorCell.size() > a_level))
    {
      std::string msg("EBAmrIce::viscousTensor !(m_viscousTensorCell.size() > a_level))");
      pout() << msg << endl;
  ...
  if (ptr == NULL)
    {
      std::string msg("EBAmrIce::viscousTensor m_viscousTensorCell[a_level] == NULL ");
      pout() << msg << endl;
      CH_assert(ptr != NULL);
      MayDay::Error(msg.c_str());
    }
...
//access the viscous tensor (cell-centered)
const LevelData<FArrayBox>* EBAmrIce::viscosityCoefficient(int a_level) const
{
  updateViscousTensor();
  if (!(m_viscosityCoefCell.size() > a_level))
    {
      std::string msg("EBAmrIce::viscosityCoef !(m_viscosityCoefCell.size() > a_level))");
      pout() << msg << endl;
  ...
  if (ptr == NULL)
    {
      std::string msg("EBAmrIce::viscosityCoef m_viscosityCoefCell[a_level] == NULL ");
      pout() << msg << endl;
      CH_assert(ptr != NULL);
      MayDay::Error(msg.c_str());
    }
...
const LevelData<FArrayBox>* EBAmrIce::dragCoefficient(int a_level) const
{
  updateViscousTensor();
  if (!(m_dragCoef.size() > a_level))
    {
      std::string msg("EBAmrIce::dragCoef !(m_dragCoef.size() > a_level))");
      pout() << msg << endl;
  ...
  if (ptr == NULL)
    {
      std::string msg("EBAmrIce::dragCoef m_dragCoefCell[a_level] == NULL ");
      pout() << msg << endl;
      CH_assert(ptr != NULL);
      MayDay::Error(msg.c_str());
    }
...
void EBAmrIce::updateViscousTensor() const
{
  CH_TIME("EBAmrIce::updateViscousTensor");
...
const LevelData<FArrayBox>* EBAmrIce::groundingLineProximity(int a_level) const
{
  ...
  if (!(m_groundingLineProximity.size() > a_level))
    {
      std::string msg("EBAmrIce::groundingLineProximity !(m_groundingLineProximity.size() > a_level)");
      pout() << msg << endl;
  ...
  if (ptr == NULL)
    {
      std::string msg("EBAmrIce::groundingLineProximity m_groundingLineProximity[a_level] == NULL)");
      pout() << msg << endl;
      CH_assert(ptr != NULL);
      MayDay::Error(msg.c_str());
    }
 ...
 */ 
void EBAmrIce::eliminateRemoteIce()
{
...
void 
EBAmrIce::implicitThicknessCorrection(Real a_dt,
				    const Vector<LevelData<FArrayBox>* >& a_sts,
				    const Vector<LevelData<FArrayBox>* >& a_bts
				    )
{

   CH_TIME("EBAmrIce::implicitThicknessCorrection");
   if (s_verbosity > 3)
     {
       pout() << "EBAmrIce::implicitThicknessCorrection" << std::endl;
     }
  ...
  if  (m_temporalAccuracy == 1)
    ...
    {    
      MayDay::Error("EBAmrIce::implicitThicknessCorrection, invalid temporal accuracy");
    }
...
void 
EBAmrIce::writePlotFile() 
{
   
  if (s_verbosity > 3) 
    { 
      pout() << "EBAmrIce::writePlotFile" << endl;
    }
...
void 
EBAmrIce::writeCheckpointFile() const
{

  if (s_verbosity > 3) 
    { 
      pout() << "EBAmrIce::writeCheckpointfile" << endl;
    }
...
void 
EBAmrIce::readCheckpointFile(HDF5Handle& a_handle)
{

  if (s_verbosity > 3) 
    { 
      pout() << "EBAmrIce::readCheckpointFile" << endl;
    }
...
/// set up for restart
void 
EBAmrIce::restart(string& a_restart_file)
{
  if (s_verbosity > 3) 
    { 
      pout() << "EBAmrIce::restart" << endl;
    }
...
Real EBAmrIce::computeTotalGroundedIce() const
{
...
void EBAmrIce::computeA(Vector<LevelData<FArrayBox>* >& a_A, 
		      ...
		      
{
  if (s_verbosity > 0)
    {
      pout() <<  "EBAmrIce::computeA" <<  std::endl;
    }
...
void EBAmrIce::computeTHalf(Vector<LevelData<FluxBox>* >& a_layerTH_half,
...
void EBAmrIce::updateTemperature(Vector<LevelData<FluxBox>* >& a_layerTH_half, 
			       Vector<LevelData<FluxBox>* >& a_layerH_half,
			       const Vector<LevelData<FluxBox>* >& a_layerXYFaceXYVel,
