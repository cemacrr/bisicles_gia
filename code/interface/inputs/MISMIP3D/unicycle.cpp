...
public:
  ...
  (const valarray<double>& cU,
   valarray<double>& fUa,
   valarray<double>& fUs) const
  {
    int nx = cU.size();
    assert(fUa.size() == nx + 1);
    assert(fUs.size() == nx + 1);
...
class L1L2VelocityModel : public VelocityModel
{
  ...
  const valarray<double> fGradS;
  const valarray<double> cGradS;
  
  valarray<double> phiTildeSqr;
  ...
  void secantSolve(double mu, double mua, double A,
		   double es, double ts, double p,
		   int niter, double tol) const
  {

    double fa, fb, t;
    fa = res(mua, A, es, ts, p);
 ...
 public:
  ...
  //vertically averaged/ surface velocities
  virtual void computeFaceVelocity
  (const valarray<double>& cU,
   valarray<double>& fUa,
   valarray<double>& fUs) const
...
//  valarray<double>& fUs) const
// {
//   int nLayer = 16; // arbitrary for now
//   valarray<double> sigma(nLayer + 1);
//   double ds = 1.0 / double(nLayer);
//   for (int l = 0; l < sigma.size(); ++l)
//     sigma[l] = ds * double(l);

//   vector<valarray<double> > fU (nLayer+1, valarray<double>(fUa.size()));
...
//   //face average of base velocity
//   faceAverage(cUb, fUa);
//   fUs = fUa;
//   //trapezium rule inegration over sigma to find integral(fU,dz)/H
//   for (int l = 0; l < nLayer; l++){
//     double s = 0.5 * (sigma[l+1] - sigma[l]);
//     fUa += s*(fU[l] + fU[l+1]);
//   }
//   //surface velocity = u(sigma = 0);
//   fUs += fU[0];
  ...
  for (int i = 0; i < fMu.size(); ++i)
    {
      ...
      const double ts = phiTildeSqr[i] * sigma;
      ...
      if (ts > es0)
	{
	  ...
	  secantSolve(fMu[i], fMua[i], fA[i], epsSqr[i], ts, p1, niter, tol);
 ...
 vector<valarray<double> >& fMu) const
{
  ...
  fMu[0] = 0.5 * pow(fA,p3)* pow(epsSqr,p2);
 ...
 valarray<double>& fMu) const
{
  ...
  vector<valarray<double> > layerFMu(nLayer + 1, fMu);
 ...
 valarray<double>& nlRes)
{
  ...
  case L1L2:
    ...
    vModel_ptr = new L1L2VelocityModel(fH,cH,fA,cA,fGradS,cGradS,dx,n);
    break;
  }
  ...
  vModel.computeFaceVelocity(cUb, fUa, fUs);
...
//advance thickness for dt years, solving the momentum equation
//to determine the velocity field

void advanceThickness(double n, double m, double L, double time, 
		      const valarray<double>& cR,
		      const valarray<double>& fA,
		      const valarray<double>& ca,
		      const valarray<double>& cC,
		      ...
		      double regPar)
{
  ...
  while (t <= time){
    ...
    double eps = 1.0e-9;
    double dt = min( 0.5 * dx / (abs(fUa).max() + 1.0), eps + time -t);
...
extern "C" {
   ...
   )
  {
	...
	advanceThickness(*n_ptr,*m_ptr,*L_ptr, *dt_ptr,
...
int main(int argc, char* argv[]){
  ...
  if (argc != 3)
    {
      cerr << "usage: " << argv[0] << " nx model " << endl;
      exit(EXIT_FAILURE);
    }
  int nx = atoi(argv[1]);
 ...
 vector<string> ccs(4);
 vector<valarray<double>* > cc(4);
 

 cc[0] = &cHend;ccs[0]="thck";
 cc[1] = &cR;ccs[1]="topg";
 cc[2] = &cS;ccs[2]="usrf";
 cc[3] = &cUb;ccs[3]="uvelb";

 stringstream ss; ss << modelname << "." << nx << ".txt";
 fout(ss.str(),ccs,cc);
