module ncdump
 ...
 subroutine nccheck(status)
  ...
  subroutine ncdump2D(x,y, thck, topg, lsrf, usrf, us, vs, umod, beta, mask, n, m)
    
    integer n, m, nc_id, time_id, x1_id, y1_id, x0_id, y0_id, &
         topg_id, lsrf_id,usrf_id, thck_id, us_id, vs_id, umod_id, beta_id, mask_id
    ...
    real(kind=8), dimension(1:n,1:m,1) :: thck, topg, lsrf, usrf,  & 
         us , vs, umod, mask
    ! cell centred arrays
    real(kind=8), dimension(1:n-1,1:m-1,1) :: beta
    ...
    call nccheck( nf90_create("pigv5.1km.nc", NF90_CLOBBER, nc_id) )
...
program t
  ...
  integer, parameter :: nx = 257, ny = 385
  ...
  real(kind=8),parameter :: minbeta = 5.0, maxbeta = 1.0e+4, cm = 1.0e-2
  real (kind = 8), dimension(1:ewn,1:nsn) :: topg, lsrf, usrf,  thck, us, vs, mask, rhostar
  real (kind = 8), dimension(1:ewn,1:nsn) :: ttopg, tlsrf, tusrf,  tthck
  real (kind = 8), dimension(wb:eb,sb:nb) :: topgb, lsrfb, usrfb,  thckb, usb, vsb , maskb
  ...
  usb = -us(wb:eb,sb:nb)
  vsb = -vs(wb:eb,sb:nb)
  !call ncdump2D(x(wb),y(sb),thckb,topgb,lsrfb,usrfb,usb,vsb,umodb,betac,maskb,nx,ny)
  !stop
  !call ncdump2D(x,y,thck,topg,lsrf,usrf,usb,vsb,thck,thck,mask,ewn,nsn)
  !stop
