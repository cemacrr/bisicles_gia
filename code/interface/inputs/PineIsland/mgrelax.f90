...
contains
 ...
 subroutine restrictnc(fine,coarse,nfx,nfy,ncx,ncy)
    ...
    integer i,j, ii, jj
    !ensure that nf = 2*(nc-1)
    if (nfx .ne. 2*(ncx-1)+1) then
       write(*,*) ' nfx =  != 2*(ncx-1) +1 '
       stop
    end if
    if (nfy .ne. 2*(ncy-1)+1) then
       write(*,*) ' nfx =  != 2*(ncx-1) +1 '
       stop
    end if
    ...
    coarse(:,ncy+1) = 0.0
     do j = 1, ncy
        do i = 1, ncx
          ...
          jj = 2*j-1
          t = 4.0*fine(ii,jj) + &
               2.0 * (fine(ii,jj-1) + fine(ii,jj+1) + fine(ii-1,jj) + fine(ii+1,jj)) + &
               (fine(ii-1,jj-1) + fine(ii-1,jj+1) + fine(ii+1,jj-1) + fine(ii+1,jj+1))
          coarse(i,j) = t / 16.0
  ...
  subroutine prolongnc(fine,coarse,nfx,nfy,ncx,ncy)
    ...
    do j = 1, ncy
       do i = 1, ncx
          ...
          fine(ii-1,jj) = 0.5 * ( coarse(i,j) + coarse(i-1,j) )
          fine(ii,jj-1) = 0.5 * ( coarse(i,j-1) + coarse(i,j) )
          fine(ii-1,jj-1) = 0.25 * ( coarse(i-1,j-1) + coarse(i,j) + coarse(i-1,j) + coarse(i,j-1))
       end do
    end do
    return
  ...
  recursive subroutine vcycle(u,v,a,nx,ny,depth,nsmooth)
    ...
    if (( mod(nx,2) .ne. 1) .or. ( mod(ny,2) .ne. 1 )) then
