...
RUNDIR=/panfs/panasas01/geog/ggslc/intel/BISICLES/BISICLES/examples/MISMIPNG/meltB/
cd $RUNDIR
...
if test -n "$(find . -maxdepth 1 -name 'chk.ghgChannel.meltB.@SMOD.@LEVlev.@SOLVER.*.2d.hdf5' -print -quit)"
    then
    LCHK=`ls -th chk.ghgChannel.meltB.@SMOD.@LEVlev.@SOLVER.*.2d.hdf5 | head -n 1`
