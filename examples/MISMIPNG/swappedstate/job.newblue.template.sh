...
RUNDIR=/panfs/panasas01/geog/ggslc/intel/BISICLES/BISICLES/examples/MISMIPNG/swappedstate/
cd $RUNDIR
...
if test -n "$(find . -maxdepth 1 -name 'chk.ghgChannel.swappedstate.@SMOD.@LEVlev.@SOLVER.*.2d.hdf5' -print -quit)"
    then
    LCHK=`ls -th chk.ghgChannel.swappedstate.@SMOD.@LEVlev.@SOLVER.*.2d.hdf5 | head -n 1`
